...
private:
	boost::int32_t/* end pos */ eat_space(const std:: vector<MYWCHAR_T> &str, boost::int32_t pos)
	{
		...
		while (si < (boost::int32_t)str.size())
        {
			MYWCHAR_T ch = str[si];
			switch (ch) {
			case ' ':
	...
	boost::int32_t/* error pos */ tokenize(std:: vector<TOKEN> *pTokens, const std:: vector<MYWCHAR_T> &script)
	{
		...
		boost::int32_t si = 0;
		while (si < (boost::int32_t)script.size()) {
			boost::int32_t spaceEnd = eat_space(script, si);
			si = spaceEnd;

			if (! (si < (boost::int32_t)script.size())) {
				break; // while
			}
...
public:
	...
	{
		assert(0 <= tokenIndex && tokenIndex < (boost::int32_t)tokens.size());
	...
	std:: vector<MYWCHAR_T> getTokenString(const TOKEN &t) const
	{
		assert(
            (0 <= t.beginPos) &&
            (t.beginPos <= t.endPos) &&
            (t.endPos <= (boost::int32_t)script.size())
            );
